...
namespace BloodDonationAPI.Controllers
{
    ...
    public class BloodBankController : Controller
    {
        ...
        [HttpPut]
        [Route("{bankID:int}")]
        public async Task<IActionResult> UpdateBank([FromRoute] int bankID, [FromBody] BloodBank updatedBank)
        {
            if (updatedBank == null)
                return BadRequest(ModelState);

            if (bankID != updatedBank.BloodBankID)
                return BadRequest(ModelState);

            dbContext.Entry(updatedBank).State = EntityState.Modified;
            ...
            {
                if (!dbContext.BloodBank.Any(e => e.BloodBankID == bankID))
                {
                    return NotFound();
                }
                else
